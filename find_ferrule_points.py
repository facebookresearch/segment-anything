...
# YOLO-format bounding box [x_center, y_center, width, height] (relative)
#yolo_box = [0.291832, 0.549231, 0.118056, 0.129712]
...
def line_from_points(p1, p2):
    """Return line in homogeneous coordinates (a, b, c) from two points"""
    x1, y1 = p1
    x2, y2 = p2
    return np.cross([x1, y1, 1], [x2, y2, 1])

def intersection(l1, l2):
    """Return intersection point of two lines in Cartesian coords"""
    pt = np.cross(l1, l2)
    if pt[2] == 0:
        return None  # Lines are parallel
    return (pt[0] / pt[2], pt[1] / pt[2])

# Collect lines in homogeneous form
lines = []
for _, pt1, pt2 in top_edges:
    pt1 = pt1.astype(np.float32)
    pt2 = pt2.astype(np.float32)

    # Extend lines again (as before)
    direction = pt2 - pt1
    norm = np.linalg.norm(direction)
    if norm == 0:
        continue
    direction = direction / norm
    pt1_ext = pt1 - direction * extension
    pt2_ext = pt2 + direction * extension

    lines.append(line_from_points(pt1_ext, pt2_ext))

# Compute intersections (6 possible pairs, choose valid 4 later)
intersections = []
...
# Optional: Filter intersections to get 4 points closest to polygon (you can also just take first 4)
# Sort by distance to centroid of polygon
intersections = np.array(intersections)
polygon_center = np.mean(pts, axis=0)
dists = np.linalg.norm(intersections - polygon_center, axis=1)
sorted_idxs = np.argsort(dists)
corner_points = intersections[sorted_idxs[:4]]

# ---- Draw corner points on the original image ----
image_with_corners = image.copy()
for pt in corner_points:
    cv2.circle(image_with_corners, tuple(np.intp(pt)), radius=6, color=(255, 0, 0), thickness=-1)

# ---- Show final result ----
#plt.figure(figsize=(10, 10))
plt.imshow(image_with_corners)
plt.title("Bright Red: Intersection Points of Extended Lines")
plt.axis('off')
plt.show()
